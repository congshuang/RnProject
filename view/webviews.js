...
class TWebView extends Component{
    ...
    componentDidMount(){
        BackHandler.addEventListener('hardwareBackPressed', this.onBackAndroid3.bind(this));
    }

    componentWillMount() {
        BackHandler.removeEventListener('hardwareBackPressed', this.onBackAndroid3.bind(this));
    }
    onBackAndroid3(){
        if(this.state.backButtonEnabled){
            this.goBack();
        }else{
            if (this.lastBackPressed3 && this.lastBackPressed3 + 5000 >= Date.now()) {
                //最近2秒内按过back键，可以退出应用。
                BackHandler.exitApp();
            }
            this.lastBackPressed3 = Date.now();
